  ...
  useEffect(() => {
    async function fetchReleaseData() {
      try {
        ...
        // Automatically start MSI download
        ...
        setError('Unable to fetch the latest release. Please try the manual download links below.');
      }
    }
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-fd-secondary/50 border border-fd-border rounded-lg p-6 mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href="https://releases.getdamp.app/releases/latest/windows-x86_64/DAMP_latest_x64_en-US.msi"
            ...
            >
              ...
              Download MSI (Fallback)
            </a>
            <a
              href="https://releases.getdamp.app/releases/latest/windows-x86_64-setup/DAMP_latest_x64-setup.exe"
      ...
      <div className="bg-fd-secondary/50 border border-fd-border rounded-lg p-8 mb-8">
        {releaseData && (
          <div className="text-center mb-6">
            <p className="text-sm text-fd-muted-foreground mb-2">Latest Version</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button
            onClick={() => releaseData && handleDownload(releaseData.platforms['windows-x86_64'].url)}
          ...
          <button
            onClick={() => releaseData && handleDownload(releaseData.platforms['windows-x86_64-setup'].url)}
            disabled={!releaseData}
            className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-fd-secondary text-fd-secondary-foreground border border-fd-border rounded-lg font-semibold hover:bg-fd-secondary/80 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Download className="w-5 h-5" />
            Download EXE
          </button>
        ...
        {releaseData && (
          <div className="mt-6 text-center text-xs text-fd-muted-foreground">
            <p>Released: {new Date(releaseData.pub_date).toLocaleDateString()}</p>
          </div>
        )}
      </div>
    </div>
