...
description: Secure HTTPS for every project with zero configuration
-icon: Lock
---

import { Step, Steps } from 'fumadocs-ui/components/steps';

DAMP provides **automatic HTTPS** for all your local projects using Caddy's built-in certificate authority. No manual configuration, no browser warnings - just secure connections out of the box.

<Callout type="success">
  **Every project gets HTTPS automatically**
  
  Caddy generates and manages SSL certificates for all your sites, giving you a production-like environment locally.
</Callout>
...
<Cards>
  <Card title="Zero Configuration" icon={<Zap />}>
    SSL certificates are generated automatically when you create a new site
  </Card>
  
  <Card title="Local Certificate Authority" icon={<Key />}>
    Caddy creates its own trusted CA for local development certificates
  </Card>
  ...
  <Card title="Browser Trust" icon={<Shield />}>
    One-time root certificate installation eliminates browser warnings
  </Card>
</Cards>

## How It Works

When you create a new site in DAMP, here's what happens:
...
For the best experience, install Caddy's root certificate once:
...
- 🛡️ **Security Testing** - Test HTTPS-only features locally
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>⚠️ Root Certificate Not Installed</td>
